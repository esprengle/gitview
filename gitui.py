...
import posix  # lstat
import urllib.parse, urllib.request, urllib.error, urllib.parse  # for push
...
class repoView(object):
    ...
    def refresh_gittle(self):
        ...
        def refresh_thread():
            if self.g:
                ...
                # self.view['tableview1'].reload()
                self.list[5] = list(
                    self.g.tracked_files - set(itertools.chain(*self.list[1:4]))
                )
    ...
    def tableview_cell_for_row(self, tableview, section, row):
        ...
        def diffact(sender):
            ...
            try:
                difftxt = [
                    x["diff"]
                    for x in self.g.diff_working()
                    if x["new"]["path"] == str(self.list[section][row])
                ]
        ...
        def diffacthtml(sender, difftype=git_diff.source.PATH):
            f = git_diff.diff_working(
                self._repo(), str(self.list[section][row]), difftype
            )
            w = ui.WebView(frame=self.view.frame)
            difftypestr = {
                git_diff.source.PATH: "working file",
        ...
        if section in (1, 4):
            ...
            if section == 1:
                ...
                b.action = functools.partial(
                    diffacthtml, difftype=git_diff.source.INDEX
                )
        if section in (0, 1, 3):
    ...
    def branch_iterator(self):
        self.g = self._get_repo()
        if self.g:
            return iter(self.g.branches.keys())

    def remotes_iterator(self):
        self.g = self._get_repo()
        if self.g:
            return iter(self.g.remotes.keys())

    def remote_branches_iterator(self):
        self.g = self._get_repo()
        if self.g:
            return iter(self.g.remote_branches.keys())
    ...
    def did_select_repo(self, sender):
        ...
        if self.g:
            ...
            if not remote:
                remote = next(self.remotes_iterator())
            self.view["remote"].text = remote
            self.view["remotebranch"].text = remote_branch
        else:
            console.hud_alert(
                "No repo found at {}.  create new repo, or clone".format(
                    self.view["repo"].text
                )
            )
            r.view["branch"].text = ""
    ...
    def _get_last_committer(self):
        try:
            ...
            if last_commit.author:
                author, author_email = (last_commit.author.split(">")[0] + "<").split(
                    "<"
                )[0:2]
            elif last_commit.committer:
                author, author_email = (
                    last_commit.committer.split(">")[0] + "<"
                ).split("<")[0:2]
                    ...
                    if os.path.exists(full_path):
                        index_entry = list(
                            index_entry_from_stat(
                                posix.lstat(full_path), tree_entry[1], 0
                            )
                        )
                    else:
    ...
    def has_uncommitted_changes(self):
        if (
            porcelain.status(self.g.path).unstaged
        ...
        if branch in self.branch_iterator():
            ...
            def checkout_sha(sender):
                build_index_from_tree(
                    repo.repo.path, indexfile, repo.repo.object_store, tree
                )
                self.refresh()
    ...
    def remote_for_head(self):
        refs = iter(self._repo().refs.as_dict().items())
        ...
        try:
            ...
            remote, remote_branch = [
                k.split("/")[-2:]
                for k, v in refs
                if v == head and k.startswith("refs/remotes")
            ][0]
    ...
    def clone_action(self, sender):
        ...
        if clipboard.get().startswith("http"):
            ...
            local = os.path.split(urllib.parse.urlparse(remote).path)[-1]
            local = local.split(".git")[0]
        d = UIDialog(
            root=self.view,
            title="Clone repo",
            items={"remote url": remote, "local path": local},
            ok_action=self.clone,
        )
    ...
    def new_action(self, sender):
        ...
        d = UIDialog(
            root=self.view, title="Clone repo", items={"repo name": ""}, ok_action=ok
        )
    ...
    def get_pass(self, netloc, callback):
        d = UIDialog(
            root=self.view,
            title="enter credentials for {}".format(netloc),
            items=dict.fromkeys(["username", "password"]),
            ok_action=callback,
        )
    ...
    def push_action(self, sender):
        ...
        branch_name = os.path.join(
            "refs", "heads", repo.active_branch
        )  #'refs/heads/%s' % repo.active_branch
        # tODO  use remote branch_name

        netloc = urllib.parse.urlparse(remote).netloc
        ...
        def push_callback(user, pw):
            ...
            if user:
                try:
                    parsedurl = urllib.parse.urlparse(remote)
                    host_with_auth = "{}:{}@{}".format(user, pw, parsedurl.netloc)

...
class UIDialog(ui.View):
    def __init__(
        ...
        header = ui.Label(
            frame=(0, 0, frame[2], rowheight * titlelines), bg_color=(0.9, 0.9, 0.9)
        )
        ...
        self.scroll = ui.ScrollView(
            frame=(
                border,
                rowheight * titlelines,
                frame[2] - 2 * border,
                min(content_height, 400),
            ),
            name="scrollview",
        )
        ...
        for item, default in items.items():
            ...
            txt = ui.TextField(
                frame=(0, curry, frame[2] - 2 * border, rowheight), name=item
            )
            if default:
                txt.text = default
        ...
        self.ok.frame = (
            border,
            self.height - rowheight - border,
            frame[2] / 2.0 - border * 2,
            rowheight,
        )
...
if __name__ == "__main__":
    ...
    d = UIDialog(
        root=r,
        title="enter some \ninfo jihbihbiybiybiybiybiybiyb0b iuh iyb uybb  iyb uy  uyuy ",
        items=dict.fromkeys(["username", "pass"]),
        ok_action=ok,
